...
namespace Plugins.VanlTools
{
    [AddComponentMenu("<XXGAME>/PrefabLightmapData")]
    [ExecuteAlways]
    public class PrefabLightmapData : MonoBehaviour
    {
        ...
        static void GenerateLightmapInfo()
        {
            ...
            foreach (var instance in prefabs)
            {
                ...
                // instance.fogStart = RenderSettings.fogStartDistance;
                // instance.fogEnd = RenderSettings.fogEndDistance;
                // instance.backgroundColor = Camera.main.backgroundColor;
                ...
                if (renderer.lightmapIndex != -1)
                {
                    ...
                    if (renderer.lightmapScaleOffset != Vector4.zero)
                    {
                        ...
                        if (info.lightmapIndex == -1)
                        {
                            ...
                            if (lightmap) lightmaps.Add(lightmap);
                            if (lightmapDir) lightmapsDir.Add(lightmapDir);
                            if (shadowMask) shadowMasks.Add(shadowMask);
                        }
